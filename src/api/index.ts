    ...
    public constructor(config: AxiosRequestConfig) {
        ...
        this.service.interceptors.request.use(
            (config: CustomAxiosRequestConfig) => {
                ...
                const Authorization = userStore.token || sessionStorage.getItem('Authorization') || ''
                //让每个请求携带自定义token
        ...
        this.service.interceptors.response.use(
            (response: AxiosResponse) => {
                ...
                //status 返回200 或者 401
                if (status === ResultEnum.SUCCESS_CODE || status === ResultEnum.OVERDUE) {
                    if (headers?.Authorization) {
                        userStore.setToken(headers.Authorization)
                ...
                // 全局错误信息拦截（防止下载文件的时候返回数据流，没有 code 直接报错） 00000
            ...
            async (error: AxiosError) => {
                ...
                if (response && response.status)
                    checkStatus(response.status, response?.data?.message || error.message || '')
